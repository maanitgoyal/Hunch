    ...
    <div className="rounded-2xl border border-white/8 bg-white/[0.03] overflow-hidden flex flex-col" style={{ height: '380px' }}>
      ...
      <form onSubmit={handleSend} className="px-3 py-3 border-t border-white/8 flex gap-2">
        <input
          ...
          maxLength={500}
          className="flex-1 px-3 py-2 rounded-xl bg-[#111118] border border-white/10 text-white text-sm placeholder-gray-600 focus:outline-none focus:border-white/30 transition-all"
        />
        <button
          type="submit"
          disabled={!text.trim() || sending}
          className="px-4 py-2 rounded-xl bg-white hover:bg-gray-100 text-black font-bold text-sm disabled:opacity-40 transition-all"
        >
          Send
        </button>
      </form>
