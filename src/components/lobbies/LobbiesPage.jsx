...
function CreateModal({ onClose, onCreated, userId }) {
  ...
  async function handleCreate(e) {
    ...
    if (!name.trim()) return
    setBusy(true)
    const { data, error } = await createLobby({ name: name.trim(), gameId: gameId || null, userId })
    setBusy(false)
...
// ─── Join lobby modal ──────────────────────────────────────────
function JoinModal({ onClose, onJoined, userId }) {
  ...
  async function handleJoin(e) {
    e.preventDefault()
    setBusy(true)
    const { data, error } = await joinLobby({ inviteCode: code, userId })
    setBusy(false)
    if (error) { setErr(error.message); return }
    onJoined(data)
  }
...
// ─── Lobby detail view ─────────────────────────────────────────
function LobbyDetail({ lobby, userId, myId, onBack, onLeft }) {
  ...
  async function handleLeave() {
    setLeaving(true)
    await leaveLobby({ lobbyId: lobby.id, userId })
    onLeft(lobby.id)
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Chat */}
      <LobbyChat lobbyId={lobby.id} userId={userId} userProfile={lobby.lobby_members?.find(m => m.user_id === userId)?.users} />
    </div>
  )
}
...
export default function LobbiesPage() {
  ...
  function handleCreated(lobby) {
    setShowCreate(false)
    load()
  }

  function handleJoined(lobby) {
    setShowJoin(false)
    load()
  }

  function handleLeft(lobbyId) {
    setSelected(null)
