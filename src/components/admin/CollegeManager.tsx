  ...
  return (
    <div className="space-y-5">
      ...
      <form onSubmit={handleCreate} className="bg-white rounded-2xl p-4 space-y-3 border border-[#e0dbd3]">
        <h3 className="text-[#1a2744] font-bold text-sm">Add College</h3>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-[#6b7a99] mb-1 block">Full name</label>
            <input className={inputCls} value={form.name} onChange={(e) => field('name', e.target.value)} placeholder="St Paul's" required />
          </div>
          <div>
            <label className="text-xs text-[#6b7a99] mb-1 block">Abbreviation</label>
        ...
        <div>
          <label className="text-xs text-[#6b7a99] mb-1 block">Logo URL (optional)</label>
          <input className={inputCls} value={form.logo_url} onChange={(e) => field('logo_url', e.target.value)} placeholder="https://…" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-[#6b7a99] mb-1 block">Primary color</label>
          ...
          <div>
            <label className="text-xs text-[#6b7a99] mb-1 block">Secondary color</label>
            <div className="flex gap-2">
      ...
      <div className="space-y-2">
        <h3 className="text-[#1a2744] font-bold text-sm">Colleges ({colleges.length})</h3>
        {colleges.map((c) => (
          <div key={c.id} className="flex items-center gap-3 bg-white rounded-xl px-3 py-2 border border-[#e0dbd3]">
            <CollegeLogo college={c} size={36} />
            <div>
              <p className="text-[#1a2744] text-sm font-semibold">{c.name}</p>
              <p className="text-[#6b7a99] text-xs">{c.abbreviation}</p>
            </div>
            <div className="ml-auto flex gap-1">
              <div className="w-4 h-4 rounded-full border border-[#e0dbd3]" style={{ backgroundColor: c.primary_color }} />
              <div className="w-4 h-4 rounded-full border border-[#e0dbd3]" style={{ backgroundColor: c.secondary_color }} />
            </div>
          </div>
        ))}
      </div>
