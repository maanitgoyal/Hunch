...
export default function ScoreEntry({ game, onDone }: ScoreEntryProps) {
  const homeAbbr = game.home_college?.abbreviation ?? 'Home'
  const awayAbbr = game.away_college?.abbreviation ?? 'Away'

  const [homeScore, setHomeScore] = useState<string | number>(game.home_score ?? '')
  const [awayScore, setAwayScore] = useState<string | number>(game.away_score ?? '')
