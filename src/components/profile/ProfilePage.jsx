...
export default function ProfilePage() {
  ...
  const netPL        = wonBets.reduce((s, b) => s + (b.payout - b.stake), 0) - lostBets.reduce((s, b) => s + b.stake, 0)
  const bestWin      = wonBets.reduce((best, b) => Math.max(best, b.payout - b.stake), 0)
  const worstLoss    = lostBets.reduce((worst, b) => Math.max(worst, b.stake), 0)

  // Current streak (consecutive wins from most recent settled bet)
  const settledDesc = [...wonBets, ...lostBets].sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
  let streak = 0
