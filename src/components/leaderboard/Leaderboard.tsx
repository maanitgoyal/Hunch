...
function PlayerRow({ row, rank, isMe }: PlayerRowProps) {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-xl px-4 py-3 transition-all
      ${isMe ? 'bg-amber-500/10 border border-amber-500/30' : 'bg-white border border-[#e0dbd3]'}
      ${rank <= 3 ? 'shadow-lg' : ''}`}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <p className="font-bold text-sm truncate text-[#1a2744]">
            {row.display_name} {isMe && <span className="text-amber-600 font-semibold">(you)</span>}
          </p>
          {streak >= 3 && (
        ...
        <p className="text-[#8a9ab0] text-xs truncate">{row.college_name}</p>
      </div>
      <div className="text-right">
        <p className="text-amber-600 font-black text-sm">🪙 {formatCoins(row.coins)}</p>
        <p className="text-[#8a9ab0] text-xs">{row.total_wins}/{row.total_bets} · {row.win_rate}%</p>
      </div>
    </div>
  )
}
...
function CollegeRow({ row, rank, isMyCollege }: CollegeRowProps) {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-xl px-4 py-3 transition-all
      ${isMyCollege ? 'bg-amber-500/10 border border-amber-500/30' : 'bg-white border border-[#e0dbd3]'}`}>
      <div className={`w-8 text-center font-black text-lg ${rankClass}`}>{icon}</div>
      <CollegeLogo
      ...
      />
      <div className="flex-1 min-w-0">
        <p className="text-[#1a2744] font-bold text-sm truncate">{row.name}</p>
        <p className="text-[#8a9ab0] text-xs">{row.member_count} member{row.member_count !== 1 ? 's' : ''}</p>
      </div>
      <div className="text-right space-y-0.5">
        <p className="text-[#1a2744] font-black text-sm">
          {row.avg_win_rate != null ? `${row.avg_win_rate}%` : '—'}
          <span className="text-[#9aaac0] font-normal text-xs ml-1">avg win rate</span>
        </p>
        <p className="text-amber-600 text-xs font-semibold">
          🪙 {row.avg_coins != null ? formatCoins(row.avg_coins) : '—'}
          <span className="text-[#9aaac0] font-normal ml-1">avg coins</span>
        </p>
      </div>
    </div>
  )
...
export default function Leaderboard() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-black text-[#1a2744]">Leaderboard</h1>
        <p className="text-[#6b7a99] mt-1">Season standings</p>
      </div>
      ...
      {profile && tab !== 'Colleges' && myRank > 0 && (
        <div className="mb-5 bg-amber-500/10 border border-amber-500/30 rounded-xl px-4 py-3 flex items-center justify-between">
          <span className="text-amber-600 font-semibold text-sm">Your rank</span>
          <div className="flex items-center gap-3">
            <span className="text-[#1a2744] font-black text-xl">#{myRank}</span>
            <span className="text-amber-600 font-bold">🪙 {formatCoins(profile.coins)}</span>
          </div>
        </div>
      )}
        ...
        <div className="mb-5 bg-amber-500/10 border border-amber-500/30 rounded-xl px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <CollegeLogo
              ...
              size={28}
            />
            <span className="text-amber-600 font-semibold text-sm">Your college</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-[#1a2744] font-black text-xl">#{myCollegeRank}</span>
            {myCollegeData.avg_win_rate != null && (
              <span className="text-amber-600 font-bold text-sm">{myCollegeData.avg_win_rate}% win rate</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        {loading ? (
          ...
          colleges.length === 0
            ? <p className="text-center text-[#8a9ab0] py-10">No colleges yet.</p>
            : colleges.map((col, i) => (
                <CollegeRow key={col.id} row={col} rank={i + 1} isMyCollege={col.name === profile?.colleges?.name} />
              ))
        ) : displayed.length === 0 ? (
          <p className="text-center text-[#8a9ab0] py-10">No players yet.</p>
        ) : (
