...
// Always-mounted canvas; each new `triggerCount` fires a fresh burst.
// Fades out naturally when all particles have fallen off screen.
function Confetti({ triggerCount }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      pieces.forEach((p) => {
        ...
        p.x   += p.vx
        p.rot += p.vrot
        // Start fading when past 70% of screen height
      ...
      if (alive > 0) {
        ...
        // All particles faded — smoothly hide canvas
        setOpacity(0)
      }
    }
    draw()
...
function LoseEffect({ triggerCount }) {
  const canvasRef = useRef(null)
  const animRef   = useRef(null)
  ...
  useEffect(() => {
    ...
    // Red screen flash: quickly fade in then out
    setFlashOp(0.18)
...
function ResultBanner({ result, label, won, onClear }) {
...
// ─── Coin Flip ──────────────────────────────────────────────
function CoinFlip({ userId, coins, onCoinsChanged, onWin, onLose }) {
...
// ─── Number Guess ────────────────────────────────────────────
function PlayingCard({ revealed, value, won, shaking }) {
  return (
...
function NumberGuess({ userId, coins, onCoinsChanged, onWin, onLose }) {
  const maxStake = Math.min(500, coins)
  const [stake, setStake]     = useState(() => Math.min(50, Math.max(10, coins)))
...
function DraftPick({ userId }) {
  ...
  useEffect(() => {
    getMyDraftPicks(userId).then(({ data }) => {
      ...
      data.forEach((d) => { p[d.pick_rank - 1] = d.colleges })
      setPicks(p)
    })
  ...
  async function handlePick(college) {
    const rank = editing
    setSaving(true)
  ...
  async function handleRemove(rank) {
...
export default function MinigamesPage() {
  ...
  function handleCoinsChanged(newCoins) {
    setCoins(newCoins)
    refreshProfile()
  }
