...
function BetCard({ bet, navigate }) {
  const game   = bet.games
  const market = bet.bet_markets
  const status = STATUS_COLORS[bet.status] ?? STATUS_COLORS.pending
  const option = market?.options?.find((o) => o.key === bet.selected_option_key)

  return (
    <button
      onClick={() => navigate(`/game/${bet.game_id}`)}
      className="w-full text-left bg-white/5 border border-white/10 rounded-2xl overflow-hidden active:scale-95 transition-transform"
    >
      ...
      <div className="px-4 py-3 flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <p className="text-gray-400 text-xs mb-0.5">{market?.description}</p>
          <p className="text-white font-medium text-sm truncate">{option?.label ?? bet.selected_option_key}</p>
        </div>
        <div className="text-right ml-3">
          <div className={`text-xs font-bold px-2 py-1 rounded-full mb-1 ${status.bg} ${status.text}`}>
            {status.label}
          </div>
          <p className="text-gray-400 text-xs">🪙 {formatCoins(bet.stake)}</p>
          {bet.status === 'pending'  && <p className="text-yellow-400 text-xs">→ {formatCoins(bet.potential_payout)}</p>}
          ...
          {bet.status === 'refunded' && <p className="text-gray-400 text-xs">refunded</p>}
        </div>
      </div>
    </button>
  )
...
export default function MyBets() {
  ...
  const netPL        = settled.reduce((acc, b) => b.status === 'won' ? acc + (b.payout - b.stake) : acc - b.stake, 0)
  const totalWagered = bets.reduce((acc, b) => acc + b.stake, 0)
  const bestWin      = won.reduce((best, b) => Math.max(best, b.payout - b.stake), 0)
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-5">
        <h1 className="text-3xl font-black text-white">My Bets</h1>
        <p className="text-gray-400 mt-1">Your betting history</p>
      </div>
      ...
      {bets.length > 0 && (
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[
            ...
            { label: 'Best win',      value: bestWin > 0 ? `+${formatCoins(bestWin)}` : '-', color: 'text-yellow-400' },
          ].map(({ label, value, color }) => (
            <div key={label} className="bg-white/5 border border-white/8 rounded-xl px-3 py-3 text-center">
      ...
      {/* Filter tabs */}
      <div className="flex gap-1.5 mb-4 bg-white/5 p-1 rounded-xl">
        {FILTERS.map((f) => {
          ...
          return (
            ...
            >
              {f}
              {count > 0 && (
                <span className={`ml-1 ${filter === f ? 'text-white/70' : 'text-gray-600'}`}>({count})</span>
              )}
            </button>
          )
        })}
      ...
      <div className="space-y-3 pb-6">
        {loading ? (
          ...
          <div className="text-center py-16 border border-dashed border-white/10 rounded-2xl text-gray-500">
            ...
            {filter === 'All' && (
              <button onClick={() => navigate('/')} className="text-yellow-400 text-sm mt-2 hover:underline">
