...
export default function BetSlip({ market, selectedOptionKey, onSuccess, onClose }) {
  const { profile, refreshProfile } = useAuth()
  const [stake, setStake] = useState(50)
  const [busy, setBusy]   = useState(false)
  const [error, setError] = useState('')

  const option        = market.options.find((o) => o.key === selectedOptionKey)
