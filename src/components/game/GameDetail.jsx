...
// ─── Prediction poll ────────────────────────────────────────
function PredictionPoll({ game, userId }) {
  const [mine, setMine]       = useState(undefined) // undefined = loading, null = not voted
  ...
  function buildCounts(rows, g) {
    const c = { [g.home_college_id]: 0, [g.away_college_id]: 0, draw: 0 }
    rows.forEach((r) => {
...
// ─── Rivalry tracker ────────────────────────────────────────
function RivalryBadge({ homeCollege, awayCollege }) {
