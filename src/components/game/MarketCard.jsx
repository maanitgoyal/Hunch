...
function PoolBar({ options, optionStakes }) {
  const total = options.reduce((sum, o) => sum + (optionStakes[o.key] ?? 0), 0)
...
export default function MarketCard({ market, existingBets = [], userCoins, onBetPlaced }) {
  ...
  // Keys the user has already bet on (may bet again)
  const userBetKeys = new Set(existingBets.map((b) => b.selected_option_key))

  const optionStakes = market.option_stakes ?? {}
  const totalPool    = Object.values(optionStakes).reduce((s, v) => s + Number(v), 0)

  function liveOdds(opt) {
          ...
          {existingBets.length > 0 && (
            <div className="space-y-1">
              {existingBets.map((bet) => {
                const optLabel = market.options.find((o) => o.key === bet.selected_option_key)?.label ?? bet.selected_option_key
                return (
                  <div
                    key={bet.id}
