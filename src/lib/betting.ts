...
export async function placeBet({ userId, marketId, gameId, selectedOptionKey, stake }: {
  userId: string; marketId: string; gameId: string; selectedOptionKey: string; stake: number
}) {
...
export async function claimBailout(userId: string) {
  const { data, error } = await supabase.rpc('claim_bailout', { p_user_id: userId })
  if (error) return { error }
  if (data?.error) return { error: { message: data.error } }
  return { data }
}
...
export async function getGames(statusFilter: string | null = null) {
  let query = supabase
    .from('games')
    .select(`
      *,
...
export async function getGameWithMarkets(gameId: string) {
  const { data, error } = await supabase
    .from('games')
    .select(`
      *,
