...
export async function createLobby({ name, gameId, userId }: { name: string; gameId: string | null; userId: string }) {
...
export async function joinLobby({ inviteCode, userId }: { inviteCode: string; userId: string }) {
  const { data: lobby, error } = await supabase
    .from('lobbies')
    .select('*')
...
export async function getMyLobbies(userId: string) {
  ...
  return { data: (data as any[]).map((m) => m.lobbies).filter(Boolean) }
}
...
export async function getLobbyLeaderboard(lobbyId: string, gameId: string | null) {
  const { data: lobby } = await getLobbyWithMembers(lobbyId)
  if (!lobby) return { data: [] }

  const members = (lobby as any).lobby_members ?? []
  ...
  const rows = members.map((m: any) => {
    const user      = m.users
    const userBets  = (bets ?? []).filter((b: any) => b.user_id === m.user_id)
    const settled   = userBets.filter((b: any) => b.status !== 'pending')
    const won       = settled.filter((b: any) => b.status === 'won')
    const totalPaid   = won.reduce((s: number, b: any) => s + (b.payout ?? 0), 0)
    const netPL       = totalPaid - settled.reduce((s: number, b: any) => s + b.stake, 0)
  ...
  rows.sort((a: any, b: any) => gameId ? b.net_pl - a.net_pl : b.coins - a.coins)
  return { data: rows }
}
